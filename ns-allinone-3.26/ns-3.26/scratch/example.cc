          ...
          for (int k = 0; k < 2; k++) //GI: 0 and 1
            {
              ...
              // channel bonding setting 
	           Ptr<RegularWifiMac> m_mac = DynamicCast<RegularWifiMac> (DynamicCast<WifiNetDevice>(apDevice.Get(0))->GetMac());  //get mac

              Ptr<MacLow> m_low = m_mac->GetLow();   // get MacLow
              m_low->EnableChannelBonding();      // enable channel bonding
              m_low->SetChannelManager(phy, 36, j, WIFI_PHY_STANDARD_80211ac);   //  channel bonding option setting 

               // channel bonding setting 
              m_mac = DynamicCast<RegularWifiMac> (DynamicCast<WifiNetDevice>(staDevice.Get(0))->GetMac());  //get mac

              m_low = m_mac->GetLow();   // get MacLow
