...
int main (int argc, char *argv[])
{
        ...
        // Set channel width
        unsigned int ap1, ap2;
        ap1 = 160;
        ap2 = 80;

        Config::Set ("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Phy/ChannelWidth", UintegerValue (ap1));   //전부 다걸림
        Config::Set ("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/Phy/ChannelWidth", UintegerValue (ap1));   //전부 다걸림
        Config::Set ("/NodeList/2/DeviceList/*/$ns3::WifiNetDevice/Phy/ChannelWidth", UintegerValue (ap2));   //전부 다걸림
        Config::Set ("/NodeList/3/DeviceList/*/$ns3::WifiNetDevice/Phy/ChannelWidth", UintegerValue (ap2));   //전부 다걸림
		...
		*/
        ...
        positionAlloc->Add (Vector (0.0, 1.0, 0.0));  //위치 추가
        ...
        if (udp)
        {
          ...
          serverApp.Stop (Seconds (simulationTime + 4));
          ...
          clientApp.Stop (Seconds (simulationTime + 4));
          ...
          */
          ...
          serverApp2.Start (Seconds (3.0));
          serverApp2.Stop (Seconds (simulationTime + 4));
          ...
          clientApp2.Stop (Seconds (simulationTime + 4));


          UdpEchoServerHelper echoServer (10);
          UdpEchoClientHelper echoClient(staNodeInterface.GetAddress(0), 10);
          ...
          back = echoServer.Install(wifiStaNode.Get(0));

          back.Start(Seconds(0.0));
          back.Stop(Seconds(2.0));

          back = echoServer.Install(wifiStaNode2.Get(0));

          back.Start(Seconds(0.0));
          back.Stop(Seconds(2.0));

          back = echoClient.Install(wifiApNode.Get(0));

          back.Start(Seconds(1.0));
          back.Stop(Seconds(2.0));

          echoClient.SetAttribute("RemoteAddress", AddressValue(staNodeInterface2.GetAddress(0)) );
          back = echoClient.Install(wifiApNode2.Get(0));
        ...
        std::cout << 0 << "\t\t\t" << ap1 << " MHz\t\t\t" << 0 << "\t\t\t" << throughput << " Mbit/s" << std::endl;
        std::cout << 0 << "\t\t\t" << ap2 << " MHz\t\t\t" << 0 << "\t\t\t" << throughput2 << " Mbit/s" << std::endl;
